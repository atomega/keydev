...
int buffer[SSD1306_LCDWIDTH * SSD1306_LCDHEIGHT / 4 ] = {
...
// Initialisation du ssd1306 en version I2C 
void ssd1306_begin( int vccstate, int i2caddr)
{
	_vccstate = vccstate;

	i2c_open_device(i2caddr); 
...
{
	// I2C
	char control = 0x00;	// Co = 0, D/C = 0
	i2c_write_16(control, c); 
}
...
{
	memset(buffer, 0,
	       (SSD1306_LCDWIDTH * SSD1306_LCDHEIGHT >> 3) * sizeof(int));
	cursor_y = 0;
	cursor_x = 0;
}

void ssd1306_drawFastHLineInternal(int x, int y, int w,  int color)
{
...
void ssd1306_drawFastVLineInternal(int x, int __y, int __h,  int color)
{
	...
	if (h >= 8) {
		if (color == INVERSE) {	// separate copy of the code so we don't
			...
			register  int val = (color == WHITE) ? 255 : 0;
...
void ssd1306_drawFastHLine(int x, int y, int w,  int color)
{
	 int bSwap = false;
	switch (rotation) {
	case 0:
...
void ssd1306_drawFastVLine(int x, int y, int h,  int color)
{
	 int bSwap = false;
	switch (rotation) {
	case 0:

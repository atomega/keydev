...
ll_i2c ma12070p(MA12070_ADDR, 1, I2C_MODE_MASTER,I2C_ADDRESS_7B);
...
void ma_setPowerMode(uint8_t mode)
{
	...
	switch(mode)
	{
		...
		case MA_PWR_MODE_1:
			...
			set_bits_range_uint16(dataTable[MA_IND_PWR_MODE_CTRL], bitStart, bitStop, valToset); 			
			break; 
...
void ma_setPowerModeProfileSetings(uint8_t setings)
{
	...
	set_bits_range_uint16(dataTable[MA_IND_PM_PROFILE_MODE], bitStart, bitStop, valToset);
}
			...
			case MA_POWER_MODE_1 : 
				switch(scheme)
				{
					case MA_POWER_SCHEME_CUSTOM : 
						...
						set_bits_range_uint16(dataTable[MA_IND_PM_PROFILE_CONF], bitStart, bitStop, valToset);
						break; 
...
uint8_t ma_setAudioInOverwrite(uint8_t enable)
{
...
uint8_t ma_setI2sFormat(uint8_t format)
{
	...
	set_bits_range_uint16(dataTable[MA_IND_PCM_PROC_SET], bitStart, bitStop, valToset);						
}
...
void ma_setI2sFrameSize(uint8_t bits)
{
	...
	set_bits_range_uint16(dataTable[MA_IND_I2S_CONFIG], bitStart, bitStop, valToset);
}
...
uint8_t ma_setVolumeCh0(uint8_t db, uint8_t lsb)
{
	if(db >= 0 && db <=255 && lsb>= 0 && lsb <= 3)
	{
		...
		set_bits_range_uint16(dataTable[MA_IND_VOL_LSB_CHX], bitStart, bitStop, lsb);	
	}
	else 
	{
		throwError(__LINE__); 
...
// TODO : Direct update via i2c 
uint8_t ma_setVolumeCh1(uint8_t db, uint8_t lsb)
{
	if(db >= 0 && db <=255 && lsb>= 0 && lsb <= 3)
	{
...
// TODO : Direct update via i2c 
uint8_t ma_setVolumeCh2(uint8_t db, uint8_t lsb)
{
	if(db >= 0 && db <=255 && lsb>= 0 && lsb <= 3)
	{
...
// TODO : Direct update via i2c 
uint8_t ma_setVolumeCh3(uint8_t db, uint8_t lsb)
{
	if(db >= 0 && db <=255 && lsb>= 0 && lsb <= 3)
	{
...
void ma_setVolumeMaster(uint8_t db, uint8_t lsb)
{
	if(db >= 0 && db <=255 && lsb>= 0 && lsb <= 3)
	{
		...
		set_bits_range_uint16(dataTable[MA_IND_VOL_LSB_MASTER], bitStart, bitStop, lsb);
